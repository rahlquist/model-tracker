...
def _toml_dump(cfg: Dict[str, Any]) -> str:
    ...
    lines.append(f"# Edit as needed, or re-run: tracker.py --setup")
    lines.append("")

    def dump_section(
        section: Dict[str, Any],
        prefix: str = "",
        flat_keys: list[tuple[str, Any]] | None = None,
    ) -> None:
        ...
        for key, val in section.items():
            if isinstance(val, dict):
                if any(isinstance(v, dict) for v in val.values()):
                    ...
                    dump_section(val, prefix=f"{prefix}{key}.")
                    lines.append("")
                else:
                    flat_keys.append((key, val))
            else:
                flat_keys.append((key, val))
